...
int OnInit()
{
    IndicatorSetString(INDICATOR_SHORTNAME, "AMD POC Trade");
                ...
                const int &spread[])
{
    ...
    double barClose = closeBuffer[0];
    double barOpen = openBuffer[0];
    double barHigh = highBuffer[0];
    double barLow = lowBuffer[0];
    ...
    if(isTradeActive)
    {
        ...
        if(isBullishTrade)
        {
            if(barLow <= slLevel || barHigh >= tpLevel)
                isTradeActive = false;
        }
        else
        {
            if(barHigh >= slLevel || barLow <= tpLevel)
                isTradeActive = false;
        }
    }
    ...
    {
        bool startsBearishManipulation = !manipulationHigh && !manipulationLow && barHigh > accHigh;
        bool startsBullishManipulation = !manipulationHigh && !manipulationLow && barLow < accLow;

        if(startsBearishManipulation)
        {
            manipulationHigh = true;
            manipulationExtremum = barHigh;
        }
        else if(startsBullishManipulation)
        {
            manipulationLow = true;
            manipulationExtremum = barLow;
        }
        ...
        if(manipulationHigh)
            manipulationExtremum = MathMax(manipulationExtremum, barHigh);
        if(manipulationLow)
            manipulationExtremum = MathMin(manipulationExtremum, barLow);

        // Calculate break confirmation metrics
        double breakBody = MathAbs(barClose - barOpen);
        double breakRange = barHigh - barLow;
        double bearCloseStrengthValue = breakRange > 0.0 ? (barHigh - barClose) / breakRange : 0.0;
        double bullCloseStrengthValue = breakRange > 0.0 ? (barClose - barLow) / breakRange : 0.0;

        // --- Bearish Setup ---
        bool bearPocBreak = manipulationHigh && profileReady &&
                            barClose < profilePoc - atr * MinPocBreakAtr &&
                            barClose < barOpen &&
...
void BuildVolumeProfile(int startBar, int endBar, double &lowBuf[], double &highBuf[], long &volBuf[])
{
    ...
    for(int row = 0; row < ProfileRows; row++)
    {
        ...
        for(int offset = 0; offset < accumulationBars && offset < 500; offset++)
        {
            ...
            double barLowVal = lowBuf[barIdx];
            double barHighVal = highBuf[barIdx];
            double barRange = barHighVal - barLowVal;
            double overlap = MathMax(0.0, MathMin(rowHigh, barHighVal) - MathMax(rowLow, barLowVal));
            double volShare = barRange > 0.0 ? (double)volBuf[barIdx] * overlap / barRange : 0.0;

            rowVol += volShare;
        }

        profileVolumes[row] = rowVol;
        maxProfileVolume = MathMax(maxProfileVolume, rowVol);
    }
